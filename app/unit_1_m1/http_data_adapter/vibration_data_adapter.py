...
def send_message_to_kafka(message):
    ...
    producer.send(kafka_config.get("topic", "sensors-u1m1"), message)
...
def on_connect(client, userdata, flags, rc):
    ...
    :return: None
    """
    client.subscribe(mqtt_config.get("topic", "unit_vibration"))
...
def mqtt_to_kafka_run():
    ...
    client_name = "smart_factory_connector_localhost"
    client = mqtt.Client(client_id=client_name)

    client.on_connect = on_connect
    client.on_message = on_message
    client.on_disconnect = on_disconnect
    # client.username_pw_set(mqtt_config.get("ACCESS_TOKEN", "488PIjyXLIBXfW5gYrTF"))

    # TODO: the queue address should be an env var
    client.connect(mqtt_config.get("host", "192.168.99.1"), mqtt_config.get("port", 1883), 60)

    client.loop_forever()


def read_config(config_path):
    """main entry point, load and validate config_u1_m1.json and call generate"""
    try:
        with open(config_path) as handle:
            ...
            logger.debug("mqtt vibration.json: {0}\nkafka vibration.json: {1}".format(mqtt_config, kafka_config))
            return [mqtt_config, kafka_config]
    except IOError as error:
        logger.error("Error opening vibration.json file '%s'" % config_path, error)
