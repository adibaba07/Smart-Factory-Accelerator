...
def on_connect(client, userdata, flags, rc):
    ...
    :return: None
    """
    client.subscribe(mqtt_config.get("topic", "sensors-u1-m1"))
...
def read_config(config_path):
    ...
    try:
        with open(config_path) as handle:
            ...
            logger.debug("mqtt config_u1_m1.json: {0}\nkafka config_u1_m1.json: {1}".format(mqtt_config, kafka_config))
            return [mqtt_config, kafka_config]
    except IOError as error:
        logger.error("Error opening config_u1_m1.json file '%s'" % config_path, error)
